...
import {
  tripStatusBadgeClassNames,
  tripStatusTopBorderClassNames,
} from "../features/trips/tripStatus.styles";
        ...
        <section className="mt-8">
          <ul className="grid gap-4 sm:grid-cols-2">
            {trips.map((trip) => (
              <li key={trip.id}>
                <Link
                  className={`block rounded-xl border border-t-2 border-slate-200 bg-white p-5 transition-colors hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700 ${tripStatusTopBorderClassNames[trip.status]}`}
                  to={`/trips/${trip.id}`}
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span
                      className={`shrink-0 rounded-full px-3 py-1 text-sm font-semibold ${tripStatusBadgeClassNames[trip.status]}`}
                    >
